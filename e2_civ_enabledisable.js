...
class E2API {
    ...
    async getCivilians() {
        console.log(`get civilians`)
        let pageSize = 10;
        let firstPageData = await this.getCiviliansPage(pageSize, 1);

        //console.log("first e page: ", firstPageData);
        let result = firstPageData.data.data;

        let pageCount = Math.ceil(parseFloat(firstPageData.data.meta.count) / pageSize);
        console.log(`  total: ${firstPageData.data.meta.count}`)
        console.log(`  processed [1/${pageCount}]`);
        for (let i = 2; i <= pageCount; i++) {
            let pageData = await this.getCiviliansPage(pageSize, i);
    ...
    async getCiviliansPage(pageSize, pageNumber) {
        let civs = await ___reactContext.api.apiClient.get(`/civilians?page=${pageNumber}&items=${pageSize}&q=&sortBy=name&sortDir=asc`);
        return civs;
    }
...
let api = new E2API();
window.civs = await api.getCivilians();
console.log(`got civs`)


async function changeEnabled(civilianId, isEnabled){
    await ___reactContext.api.apiClient.patch(`/civilians/${civilianId}`, { enabled: isEnabled});
}
async function changeThreshold(civilianId, percentage){
    await ___reactContext.api.apiClient.patch(`/civilians/${civilianId}`, { thresholdPercentage: percentage});
}

let thresholdPercentage = config.dispenseOnlyFull ? 100 : 0;
for(let i = 0; i<civs.length; i++){
    let civ = civs[i];

    if(civ.attributes.state === "idle"){
        let enabled = false;
        switch(civ.attributes.occupation){
            case "ether_dispenser":
                enabled = config.enableDispenser;
                break;
            case "cydroid_technician":
                enabled = config.enableTechnician;
                break;
            case "raid_commander":
                enabled = config.enableCommander;
                break;
        }

        if(civ.attributes.enabled !== enabled){
            console.log(`civ [${civ.attributes.name}] (${civ.attributes.occupation}) change enabled to: ${enabled}`);
            await changeEnabled(civ.id, enabled);
            await helper.sleep(helper.getWaitTime(i,500));
        }

        if(civ.attributes.occupation === "ether_dispenser" && civ.attributes.thresholdPercentage !== thresholdPercentage){
            await changeThreshold(civ.id,thresholdPercentage);
        }
    }
}
