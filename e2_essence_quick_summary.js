...
class ETransactions {
    ...
    async getAllEssenceTransactions () {
        let result = [];
        let types = ["MAIL_ATTACHMENT_CLAIM", "RESOURCE_CLAIM_STAKE_REFUND", "RESOURCE_CLAIM_STAKE", "RESOURCE_CLAIM_STAKE_REWARD", "JEWEL_CRAFTING"];
        for (let i = 0; i < types.length; i++) {
            let transactionType = types[i];
            result = result.concat(await this.getAllEssenceTransactionsForType(transactionType));
    ...
    async getAllEssenceTransactionsForType (transactionType) {
        console.log(`get transactions, type: [${transactionType}]`)
        let pageSize = 100;
        let firstPageData = await this.getEssenceTransactionPage(pageSize, 1, transactionType);

        //console.log("first e page: ", firstPageData);
        let result = firstPageData.data.data;

        let pageCount = Math.ceil(parseFloat(firstPageData.data.meta.totalCount) / pageSize);
        console.log(`  transactions: ${firstPageData.data.meta.totalCount}`)
        console.log(`  processed [1/${pageCount}]`);
        for (let i = 2; i <= pageCount; i++) {
            let pageData = await this.getEssenceTransactionPage(pageSize, i, transactionType);
    ...
    async getEssenceTransactionPage (pageSize, pageNumber, transactionType) {
        let transactions = await ___reactContext.api.apiClient.get(`/transactions/resources?page=${pageNumber}&items=${pageSize}&ticker=ESNC&type=${transactionType}`);
...
window.essenceTransactions = await etr.getAllEssenceTransactions();
console.log(`Getting transactions finished: ${helper.getFormattedTime()}`);

function total (transactions) {
    return transactions.reduce((a, b) => a + parseFloat(b.amount), 0);
}

let essenceStakedTransactions = essenceTransactions.filter(e => e.balanceChangeType === "resource_claim_stake");
let essenceStakeRewardTransactions = essenceTransactions.filter(e => e.balanceChangeType === "resource_claim_stake_reward");
let essenceStakeRefundTransactions = essenceTransactions.filter(e => e.balanceChangeType === "resource_claim_stake_refund");
let essenceStakeReimburseTransactions = essenceTransactions.filter(e => e.balanceChangeType === "mail_attachment_claim");
let essenceCraftingTransactions = essenceTransactions.filter(e => e.balanceChangeType === "jewel_crafting");

let essenceStakedAmount = -total(essenceStakedTransactions);
let essenceRewardAmount = total(essenceStakeRewardTransactions);
let essenceRefundsAmount = total(essenceStakeRefundTransactions);
let essenceReimburseAmount = total(essenceStakeReimburseTransactions);
let essenceCraftingAmount = total(essenceCraftingTransactions);

let max = Math.max.apply([essenceStakedAmount,essenceRewardAmount,essenceRefundsAmount,essenceReimburseAmount,essenceCraftingAmount]);
let maxLength = max.toString().length +2;

console.log(`staked in             : %c-${essenceStakedAmount.toFixed(2).padStart(maxLength," ")}`,"color:magenta");
console.log(`staked reward         : %c+${essenceRewardAmount.toFixed(2).padStart(maxLength," ")}`,"color:orange");
console.log(`staked refund         : %c+${essenceRefundsAmount.toFixed(2).padStart(maxLength," ")}`,"color:lime");
console.log(`staked reimbursementts: %c+${essenceReimburseAmount.toFixed(2).padStart(maxLength," ")}`,"color:deepskyblue");

let balanceChange = -essenceStakedAmount + essenceRewardAmount + essenceRefundsAmount + essenceReimburseAmount;
console.log(`overall change from staking: %c${balanceChange.toFixed(2)}`,"color:gold");

console.log(`spent on crafting jewels: %c${essenceCraftingAmount.toFixed(2)}`,'color:red');
